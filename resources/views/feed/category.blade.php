...
@section('title', env('APP_PROJECTNAME') . ' - ' . $category->name)
...
<script>
        ...
        function fetchPosts()
        {
            ...
            if (window.mayFetchPosts) {
                ...
                window.vue.ajaxRequest('GET', '{{ url('/fetch/posts') }}?type=' + window.vue.getPostFetchType() + '&category={{ $category->id }}' + ((window.paginate !== null) ? '&paginate=' + window.paginate : ''), {}, function(response){
